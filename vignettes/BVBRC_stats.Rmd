...
fetchCompleteBVBRCMetadataAPI <- function(
    batch_size = 25000,
    verbose = TRUE
) {

    base_url <- "https://www.bv-brc.org/api/genome/?superkingdom=Bacteria"

    if (isTRUE(verbose)) {
        message("Fetching field definitions...")
    }
    ...
    sample_rec <- jsonlite::fromJSON(
        httr2::resp_body_string(resp)
    )

    attrs <- names(sample_rec)

    if (length(attrs) == 0) {
        stop("Unable to determine genome attributes.")
    }

    if (isTRUE(verbose)) {
        message(sprintf("Found %s attributes.", length(attrs)))
    }

    total_records <- as.numeric(
        sub(
            ".*/",
            "",
            httr2::resp_headers(resp)[["content-range"]]
        )
    )

    if (isTRUE(verbose)) {
        message(sprintf(
            "Server reports %s total records.",
...
is_placeholder_na <- function(x) {
  ...
  stringr::str_trim(x) %in% missing_tokens
}
  ...
  df |>
    dplyr::mutate(dplyr::across(
      ...
      ~ {
        ...
        if (is.character(x)) {
          x <- fix_utf8(x)
          x <- stringr::str_squish(x) # trim + collapse internal spaces
...
summarize_block <- function(df, cols, block_name) {
  ...
  df |>
    dplyr::transmute(
      .row_any_present = dplyr::if_any(dplyr::all_of(cols), ~ !is.na(.x)),
      .row_all_missing = dplyr::if_all(dplyr::all_of(cols), is.na)
    ) |>
    dplyr::summarise(
      block = block_name,
      n_total = dplyr::n(),
...
bvbrc <- fetchCompleteBVBRCMetadataAPI()
...
col_stats <- tibble::tibble(column = colnames(bvbrc_clean)) |>
  dplyr::mutate(
    ...
    n_missing = purrr::map_int(column, ~ sum(is.na(bvbrc_clean[[.x]]))),
    n_present = n_total - n_missing,
    pct_missing = 100 * n_missing / n_total,
    n_distinct_non_missing = purrr::map_int(column, ~ dplyr::n_distinct(bvbrc_clean[[.x]], na.rm = TRUE))
  ) |>
  dplyr::arrange(dplyr::desc(pct_missing), column)
...
diag_table <- tibble::tibble(column = diag_cols) |>
  dplyr::mutate(
    n_total = nrow(bvbrc_clean),
    n_missing = purrr::map_int(column, ~ sum(is.na(bvbrc_clean[[.x]]))),
    n_present = n_total - n_missing,
...
### Potential Species for amReh
```{r priority-pathogens}
species_for_amreh <- bvbrc_clean |> 
  dplyr::distinct(species, antimicrobial_resistance, genome_id, antimicrobial_resistance_evidence, genome_quality, genome_status, checkm_completeness) |>
  dplyr::filter(antimicrobial_resistance_evidence == "AMR Panel", genome_quality == "Good", genome_status %in% c("WGS", "Complete"), as.numeric(checkm_completeness) > 98, !is.na(species), !is.na(antimicrobial_resistance)) |>
  dplyr::distinct(species, genome_id) |> 
  dplyr::count(species) |> 
  dplyr::arrange(n) |>
  dplyr::filter(n > 20 & n < 1000) |>
  dplyr::pull(species)

cat(
  "The recommended list of bacteria to be included in Zenodo are:\n",
  paste(species_for_amreh, collapse = "\n"),
  "\n"
)
```
