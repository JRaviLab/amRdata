...
## Potential Species for Exploring the Package

Candidate species were selected using the following criteria:

- `antimicrobial_resistance_evidence` = "AMR Panel"
- `genome_quality` = "Good"
- `genome_status` = "WGS" or "Complete"
- `checkm_completeness` > 98
- `genome_count` between 20 and 1,000
...
species_count_summary <- bvbrc_clean |>
  ...
  dplyr::filter(
    ...
    as.numeric(checkm_completeness) > 98,
    !is.na(species),
...
species_counts_named <- stats::setNames(
  species_count_summary$genome_count,
  species_count_summary$species
)

save(
  species_counts_named,
  file = "data/species_count_summary.rda"
)



species_for_amreh <- species_count_summary |>
  dplyr::pull(species)
