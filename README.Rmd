...
taxon ID, downloads the correspondinggenomes and AST data from BV‑BRC, constructs 
...
The pipeline is made up of 6 primary processes:

**1. BV‑BRC metadata → 2. BV-BRC genomes → 3. Panaroo pangenome →  
4. CD‑HIT protein clusters → 5. Pfam domain extraction → 6. Database formatting
...
for Shigella flexneri genomes with paired AST metadata. 
...
runDataProcessing("amR_data/data/Shigella_flexneri/Sfl.duckdb")

# 2. Train models
runMLmodels("amR_data/data/Shigella_flexneri/Sfl_parquet.duckdb")
